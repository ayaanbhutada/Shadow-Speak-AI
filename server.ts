...
// Helper to parse JSON from AI output even with markdown blocks, think tags, or minor syntax glitches
function extractJsonFromText(rawText: string): any {
  if (!rawText || !rawText.trim()) throw new Error('Empty response from AI');

  // Strip reasoning blocks like <think>...</think> (e.g. DeepSeek R1 / Qwen models on Groq)
  ...
  try {
    ...
    throw new Error(`Failed to parse valid JSON: ${initialErr instanceof Error ? initialErr.message : String(initialErr)}`);
  }
}

async function callGroqChatOnServer(
  groqKey: string,
  modelId: string,
          ...
          const systemMsg = `You are the AAC prediction engine for "Shadow Speak AI". Output strictly valid JSON object matching this schema:
{
  "responses": [
    { "id": "p1", "text": "Exact short sentence to speak", "tag": "Short Label", "details": "Expanded details" }
          ...
          const parsed = await callGroqChatOnServer(
            groqKey,
            resolvedGroqModel,
            [
          ...
          if (parsed && parsed.responses && Array.isArray(parsed.responses) && parsed.responses.length > 0) {
          ...
          console.error("Groq API call error on server:", groqErr);
        }
      }
    }
...
app.post("/api/expand-response", async (req, res) => {
  try {
    ...
    const isEnergy = /\b(tired|rest|energy|sleep|exhausted|quiet|break|nap|pain|lie down|relax|hungry)\b|आराम|थक|नींद|एनर्जी|रेस्ट|विश्राम|शांति/i.test(`${responseText} ${tag}`);
          ...
          const systemMsg = `You are the AAC response expansion engine for "Shadow Speak AI". You must respond with a valid JSON object matching this schema:
{
  "options": [
    { "label": "Descriptive Label", "text": "Detailed sentence elaborating on choice...", "tone": "warm" }
          ...
          if (parsed && parsed.options && Array.isArray(parsed.options) && parsed.options.length > 0) {
            return res.json({ options: parsed.options });
          }
        } catch (groqExpErr) {
          console.error("Groq expansion error on server:", groqExpErr);
        }
      }
    }
    ...
    return res.json({
      options: generateFallbackExpandedOptions(req.body?.responseText, req.body?.transcript, req.body?.userProfile?.language, req.body?.tag)
    });
  }
});
